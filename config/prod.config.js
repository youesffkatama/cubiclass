...
const CONFIG = {
  ...
  MONGODB_URI:
    process.env.MONGODB_URI || "mongodb://localhost:27017/scholar_ai",
...
if (CONFIG.NODE_ENV === "production") {
  if (CONFIG.JWT_SECRET === "fallback_jwt_secret_for_development") {
    console.error(
      "❌ JWT_SECRET is using default value in production - this is insecure!",
    );
    process.exit(1);
  }
  ...
  if (CONFIG.JWT_REFRESH_SECRET === "fallback_refresh_secret_for_development") {
    console.error(
      "❌ JWT_REFRESH_SECRET is using default value in production - this is insecure!",
    );
    process.exit(1);
  }
}
