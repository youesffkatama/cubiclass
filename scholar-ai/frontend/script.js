...
// Add this after AppState definition (around line 30)
// ==========================================
// API CONFIGURATION
// ==========================================
const API_CONFIG = {
...
const AuthModule = {
    init: () => {
        ...
        const user = Utils.loadFromStorage('scholar_user');
        if (user) {
            AppState.user = user;
            AuthModule.loadApp();
        }
    ...
    handleLogin: async (e) => {
        ...
        try {
            ...
            Utils.saveToStorage('scholar_user', response.data.user);
...
const ClassModule = {
    init: () => {
        ...
        AppState.classes = Utils.loadFromStorage('scholar_classes', [
            { id: 'demo-1', name: 'Advanced Calculus', description: 'Math 401', color: 'green' },
            { id: 'demo-2', name: 'Quantum Physics', description: 'Physics 301', color: 'blue' }
        ]);
    ...
    createClass: () => {
        ...
        const newClass = {
            ...
            color: selectedColor ? selectedColor.getAttribute('data-color') : 'green',
            created: Date.now()
        };
        ...
        setTimeout(() => ClassModule.openClass(newClass.id), 500);
    },
...
const AIModule = {
    ...
    sendMessage: async () => {
        ...
        try {
            // REAL API CALL - Add this new code
            if (PDFModule.currentPdf && PDFModule.currentPdf.id) {
                const response = await API.post('/intelligence/chat/stream', {
                    query: text,
                    nodeId: PDFModule.currentPdf.id,
                ...
                aiMsg.innerHTML = `
                    <div class="message-avatar"><i class="fas fa-robot"></i></div>
                    <div class="message-content">
                        <div class="message-bubble">${response.data.message || 'Response received'}</div>
                    </div>
                `;
                messagesContainer.appendChild(aiMsg);
            } else {
                // Fallback to demo if no PDF
                document.getElementById('typingIndicator')?.remove();
                const response = AIModule.generateResponse(text);
                ...
                await Utils.typeWriter(bubble, response, 15);
            }
...
const FlashcardModule = {
    ...
    generateFromPdf: (pdf) => {
        ...
        FlashcardModule.cards = [
            { question: 'What is the derivative of x²?', answer: '2x' },
            { question: 'Define entropy in thermodynamics', answer: 'A measure of disorder or randomness in a system' },
            { question: 'What is the Pythagorean theorem?', answer: 'a² + b² = c² for right triangles' },
            { question: 'Who discovered calculus?', answer: 'Isaac Newton and Gottfried Leibniz independently' },
...
const ActivityModule = {
    init: () => {
        AppState.activities = Utils.loadFromStorage('scholar_activities', [
            { 
                type: 'quiz', 
                title: 'Calculus Quiz Completed', 
                score: '95%', 
