...
export const PDFModule = {
    ...
    handleUpload: async (event) => {
        ...
        try {
            ...
            const progressContainer = document.getElementById('pdfProcessingProgress');
    ...
    showPdfDashboard: async (pdf) => {
        ...
        if (status === 'QUEUED' || status === 'PROCESSING') {
            ...
            if (progressText) progressText.textContent = `${statusResponse.data.progress || 0}%`;
            if (progressBar) progressBar.style.width = `${statusResponse.data.progress || 0}%`;
        } else {
    ...
    launchTool: (toolName) => {
        Utils.showToast(`Launching ${toolName} for ${PDFModule.currentPdf.meta.originalName}`, 'info');
