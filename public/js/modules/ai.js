    ...
    try {
      const response = await fetch("/api/v1/intelligence/chat/stream", {
        ...
        body: JSON.stringify({
          query,
          nodeId:
            AIModule.activeTool === "pdfchat" && PDFModule.currentPdf
              ? PDFModule.currentPdf._id
              : undefined,
      ...
      let assistantMessageElement = AIModule.addMessage("assistant", "", {
        pending: true,
      });
      ...
      AIModule.addMessage("assistant", `Error: ${error.message}`, {
        error: true,
      });
