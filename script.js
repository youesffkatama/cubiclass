  ...
  baseURL:
    ...
    window.location.hostname.includes("app.github.dev")
      ...
      : window.location.hostname.includes("localhost") || window.location.hostname.includes("127.0.0.1")
        ...
        : window.location.origin + "/api/v1", // For deployed environments
  ...
  animate: () => {
    ...
    ParticleSystem.particles.forEach((particle, index) => {
      ...
      if (distance < ParticleSystem.mouse.radius) {
        const force =
          (ParticleSystem.mouse.radius - distance) /
          ParticleSystem.mouse.radius;
...
const CursorEffect = {
  init: () => {
    ...
    // Scale on hover
    const hoverElements =
      "a, button, .nav-link, .feature-card, .btn-icon, .chip, input, textarea, select, .btn-primary, .btn-glass";
...
const SmoothScroll = {
  init: () => {
    ...
    document
      .querySelectorAll(
        ".feature-card, .activity-item, .timeline-item, .stat-tile",
      )
      .forEach((el) => {
...
const TiltEffect = {
  init: () => {
    const tiltElements = document.querySelectorAll(
      ".feature-card, .pdf-tool-card, .chart-card, .metric-card, .stat-tile",
    );
...
const GlowEffect = {
  init: () => {
    document
      .querySelectorAll(".btn-primary, .btn-white, .btn-send")
      .forEach((btn) => {
...
const NotificationSystem = {
  init: () => {
    ...
    const notifBtn = document.querySelector(
      '.topbar-right .btn-icon[title="Notifications"]',
    );
    if (notifBtn) {
...
const ProgressiveLoader = {
  ...
  showSkeleton: (container) => {
    ...
    container.innerHTML = Array(parseInt(count))
      .fill(0)
      .map(
        () => `
...
const EnhancedClassModule = {
  ...
  setupClassView: (classId) => {
    ...
    document
      .querySelector(".btn-new-material")
  ...
  openMaterialModal: () => {
    ...
    modal.querySelectorAll(".material-type").forEach((type) => {
      type.addEventListener("click", () => {
        modal
          .querySelectorAll(".material-type")
  ...
  createMaterial: () => {
    ...
    const type = document.querySelector(".material-type.selected")?.dataset
      .type;
...
const AIModule = {
  ...
  init: () => {
    ...
    // Bind model selector
    document
      .getElementById("aiModelSelector")
  ...
  sendMessage: async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_CONFIG.baseURL}/intelligence/chat/stream`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
const PDFModule = {
    ...
    const checkStatus = async () => {
      try {
        ...
        if (response.data.progress !== undefined) {
          ...
          if (progressText)
            progressText.textContent = `${response.data.progress}%`;
        }
  ...
  showPdfDashboard: (pdf) => {
    ...
    document.getElementById("flashcardCount").textContent =
      `${pdf.flashcards} Concepts Generated`;
    document.getElementById("quizCount").textContent =
      `${pdf.questions} Practice Questions`;
...
const StatsModule = {
      ...
      async loadPerformanceData() {
        try {
          ...
          const values = labels.map((date) => {
            ...
            return (
              (dayData.chat || 0) + (dayData.study || 0) + (dayData.quiz || 0)
            );
          });
  ...
  animateMetrics: () => {
    ...
    if (sessions)
      Utils.animateNumber(sessions, 0, AppState.stats.sessions, 1500);
...
const i18nModule = {
  ...
  translations: {
    ...
    fr: {
      ...
      feature_tutor_desc:
        "Obtenez une aide instantanée sur n'importe quel sujet",
  ...
  translate: (key) => {
    ...
    return (
      i18nModule.translations[lang][key] ||
      i18nModule.translations["en"][key] ||
      key
    );
  },
  ...
  setLanguage: (langCode) => {
    ...
    Utils.showToast(
      `Language changed to ${i18nModule.getLanguageName(langCode)}`,
      "success",
    );
  },
  ...
  renderActiveUsers: () => {
    ...
    container.innerHTML = `
            <div class="active-users-list">
                ${CollaborationRealTimeModule.activeUsers
                  .map(
                    (user) => `
...
const VoiceCommandModule = {
  ...
  init: () => {
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      VoiceCommandModule.recognition.lang =
        i18nModule.currentLanguage || "en-US";
...
const NoteModule = {
  ...
  renderNotesList: () => {
    ...
    container.innerHTML = NoteModule.notes
      .map(
        (note) => `
  ...
  function startUserUpdatesPolling() {
    ...
    setInterval(async () => {
      try {
        ...
        if (AppState.user && AppState.user.dna.xp !== newUser.dna.xp) {
          ...
          if (xpDiff > 0) {
            ...
            if (newUser.dna.level > AppState.user.dna.level) {
              Utils.showToast(
                `🎉 Level Up! You're now level ${newUser.dna.level}!`,
                "success",
              );
            } else {
        ...
        const userName = document.getElementById("userName");
        if (userName)
          userName.textContent = AppState.user.username || "Student";
      } catch (error) {
    ...
    setInterval(async () => {
      try {
        ...
        if (
          ...
          response.data.recentActivity.length > 0
        ) {
          // Update activity feed if there are new activities
          if (
            typeof ActivityModule !== "undefined" &&
            ActivityModule.renderFeed
          ) {
            ActivityModule.renderFeed();
          }
...
console.log(
  "🚀 Scholar.AI Extended v2.5 - Complete with i18n, Gamification, Voice Commands, Study Timer, and Notes!",
);
